...
def run(data,
        ...
        for si, pred in enumerate(out):
            ...
            if save_txt:
                ...
                for *xyxy, conf, cls in predn.tolist():
                    ...
                    # xmax = round(xyxy[2], 3)
                    # ymax = round(xyxy[3], 3)
                    # boundingbox.append(xmin, ymin, xmax, ymax)
                    # print(boundingbox)
            ...
            if save_json:
                ...
                for p, b in zip(pred.tolist(), box.tolist()):
                    ...
                    if (round(p[4], 2) > 0.5):
                        temp_object.append({'class': names[p[5]],
                            'position': [x_min, y_min, x_max, y_max],
                            'confidence_score': round(p[4], 2)}) 

                jdict.append({'image_id': imageid, 
    ...
    if save_json and len(jdict):
        ...
        with open(pred_json, 'w') as f:
            dididict = {}
            dididict['annotations'] = jdict
            json.dump(dididict, f)
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default='../test/test.yaml', help='dataset.yaml path')
